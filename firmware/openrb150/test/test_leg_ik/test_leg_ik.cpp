...
void test_home_foot_maps_to_urdf_zero() {
...
void test_fk_round_trip_nominal() {
  ...
  // Reconstruct raw angles from the returned URDF-zero-relative commands and
  // run FK; it must reproduce the requested planted target exactly.
  float fx, fy, fz;
  ik.forwardRaw(r.coxa, r.femur + ik.femurRest(),
                r.tibia + ik.tibiaRest(), fx, fy, fz);
...
void test_unreachable_still_returns_clamped_angles() {
  ...
  TEST_ASSERT_FALSE(r.reachable);
  // acos clamps cos_k to 1 -> raw tibia 0 -> relative tibia = -rest.
  TEST_ASSERT_FLOAT_WITHIN(1e-3f, -ik.tibiaRest(), r.tibia);
  // No NaNs leaked.
...
void test_leg_ik_home_params_shift_rest_offset() {
  ...
  // Default maps the documented home foot to URDF zero.
  ...
  // ...and maps ITS configured home (10 mm lower) to URDF zero.
...
void test_solve_body_home_is_urdf_zero_all_legs() {
  RobotConfig cfg;
  defaultRobotConfig(cfg);
  BodyKinematics bk(cfg);
...
int main(int, char**) {
  UNITY_BEGIN();
  RUN_TEST(test_home_foot_maps_to_urdf_zero);
  ...
  RUN_TEST(test_solve_body_home_is_urdf_zero_all_legs);
